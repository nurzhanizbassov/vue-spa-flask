<template>
  <div>
    ...
    <section class="section">
      <form id="add-game-form" @submit="add">
        <div class="container">
          <b-field
...
export default {
  ...
  methods: {
    add() {
      console.log('UAC. addGame. store', this.$store);
      console.log('UAC. addGame. currentUser',
        this.$store.getters.getCurrentUser);
      let userData = this.$store.getters.getCurrentUser;
      console.log('UAC. addGame. userData:', userData);
      this.$store
        .dispatch('addGame', {
          nameKz: this.nameKz,
          nameRu: this.nameRu,
        ...
        .then(response => {
          this.$toasted.global.toasted_success({
            message: this.$i18n.translate('record_added')
          });
        });
    },
    back () {
